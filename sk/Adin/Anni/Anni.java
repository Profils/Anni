...
public final class Anni extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if ((getServer().getPluginManager().getPlugin("Vault") instanceof Vault))
        {
		@SuppressWarnings("rawtypes")
		RegisteredServiceProvider service = getServer().getServicesManager().getRegistration(Economy.class);
          if (service != null)
          {
            economy = (Economy)service.getProvider();
            if (economy != null) {
              getLogger().info("Vault detected and loaded, economy available.");
            }
          }
        }
        reset();
    ...
    public void onSecond() {
        ...
        if (time == -5L) {
            ...
            voting.end();
            getServer().broadcastMessage(prefix + "Голосование закрыто!");
            ...
            getServer().broadcastMessage(prefix + " §7Карта §e" + WordUtils.capitalize(winner) + " §7Выбрана!");
            loadMap(winner);
    ...
    public boolean resetGame() {
		reset(); // TODO Auto-generated method stub
		return false;
	}
    ...
    public void listTeams(CommandSender sender) {
        sender.sendMessage("§8============[ §eКоманда §8]============");
        for (GameTeam t : GameTeam.teams()) {
            int size = 0;
            ...
            if (size != 1) {
                sender.sendMessage(t.coloredName() + " §7- " + size + " §6Игрок");
            } else {
                sender.sendMessage(t.coloredName() + " §7- " + size + " §6Игроков");
            }
        }
	...
	public void joinTeam(Player player, String team) {
        ...
        if (meta.getTeam() != GameTeam.NONE && !player.hasPermission("anni.bypass.teamlimitor")) {
            player.sendMessage(prefix +  " §cВы не можете изменить команду!");
            return;
        }
        ...
        try {
            ...
            player.sendMessage(prefix + ChatColor.RED + " §cНеизвесная команда!");
            listTeams(player);
            return;
        }
        ...
        if (Util.isTeamTooBig(target)
            ...
            player.sendMessage(prefix + " §cНет места в команде!");
            return;
        }

        if (target.getNexus() != null) {
            if (target.getNexus().getHealth() == 0 && getPhase() > 1) {
                player.sendMessage(prefix + " §cНельзя выбрать команду без Nexus!");
                return;
            }
        }
        ...
        if (getPhase() > lastJoinPhase
            ...
            player.kickPlayer(prefix + ChatColor.RED
                    + " Вы не можете войти в этой фазе!");
            return;
        }
        ...
        player.sendMessage(prefix + " §7Выбрана команда "
                + target.coloredName());
    ...
    public void Team(final Player p) {
    	@SuppressWarnings("deprecation")
		IconMenu menu = new IconMenu("§0§l§nВыбор Команды", 9, new IconMenu.OptionClickEventHandler() {
            @Override
